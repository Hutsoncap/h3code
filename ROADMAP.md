...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 merged slices: [#196](https://github.com/Hutsoncap/h3code/pull/196) + [#197](https://github.com/Hutsoncap/h3code/pull/197) + [#198](https://github.com/Hutsoncap/h3code/pull/198) + [#199](https://github.com/Hutsoncap/h3code/pull/199) — thread-scoped terminal state/store wiring, composer voice lifecycle, runtime/interaction-mode plus persisted thread-settings bindings, and the environment-mode binding all now live on `main`; live lane: `/Users/hutson/Code/h3code-p0.3-chatview-composer-model-bindings` (`codex/p0.3-chatview-composer-model-bindings`) / draft PR [#200](https://github.com/Hutsoncap/h3code/pull/200) against `main` — the provider-model / traits / fast-mode extraction has local `bun fmt`, `bun lint`, and `bun typecheck` green, and CI is passing everywhere except `Build Desktop Pipeline`, which failed in `bun install` because Electron's postinstall hit an external `504 Gateway Time-out`; [apps/web/src/components/ChatView.tsx](apps/web/src/components/ChatView.tsx) is still ~5.6k lines on the live branch, so Phase 1 remains blocked until more seams land |
