...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 live lane: `/Users/hutson/Code/h3code` (`codex/p0.3-chatview-terminal-state-bindings`) / draft PR [#196](https://github.com/Hutsoncap/h3code/pull/196) — CI is fully green and the slice is merge-ready after pulling thread-scoped terminal state/store wiring into `useChatTerminalBindings`; next stacked slice is active in `/Users/hutson/Code/h3code-p0.3-chatview-voice-controller` (`codex/p0.3-chatview-voice-controller`) to extract the composer voice-control block out of [apps/web/src/components/ChatView.tsx](apps/web/src/components/ChatView.tsx), which is still ~5.9k lines on the `#196` branch, so Phase 1 remains blocked until more seams land |
