...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 merged slice: [#196](https://github.com/Hutsoncap/h3code/pull/196) — thread-scoped terminal state/store wiring now lives in `useChatTerminalBindings` on `main`; live lane: `/Users/hutson/Code/h3code-p0.3-chatview-voice-controller` (`codex/p0.3-chatview-voice-controller`) / PR [#197](https://github.com/Hutsoncap/h3code/pull/197) against `main` — the composer voice-controller extraction in `useComposerVoiceController` is fully green and merge-ready; stacked lane: `/Users/hutson/Code/h3code-p0.3-chatview-thread-settings-bindings` (`codex/p0.3-chatview-thread-settings-bindings`) / draft PR [#198](https://github.com/Hutsoncap/h3code/pull/198) against `#197` — CI has started for the runtime/interaction-mode and persisted thread-settings extraction, and [apps/web/src/components/ChatView.tsx](apps/web/src/components/ChatView.tsx) is down to ~5.6k lines on the stacked branch, so Phase 1 remains blocked until more seams land |
