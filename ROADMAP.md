...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 merged slices: [#196](https://github.com/Hutsoncap/h3code/pull/196) + [#197](https://github.com/Hutsoncap/h3code/pull/197) — thread-scoped terminal state/store wiring now lives in `useChatTerminalBindings`, and the composer voice lifecycle now lives in `useComposerVoiceController` on `main`; live lane: `/Users/hutson/Code/h3code-p0.3-chatview-thread-settings-bindings` (`codex/p0.3-chatview-thread-settings-bindings`) / draft PR [#198](https://github.com/Hutsoncap/h3code/pull/198) against `main` — the runtime/interaction-mode and persisted thread-settings extraction has been rebased and republished on current `main` so CI can rerun on a fresh head; stacked lane: `/Users/hutson/Code/h3code-p0.3-chatview-env-mode-bindings` (`codex/p0.3-chatview-env-mode-bindings`) / draft PR [#199](https://github.com/Hutsoncap/h3code/pull/199) against `#198` — the `onEnvModeChange` extraction is now being restacked on top of that refreshed front lane, and [apps/web/src/components/ChatView.tsx](apps/web/src/components/ChatView.tsx) is down to ~5.6k lines on the stacked branch, so Phase 1 remains blocked until more seams land |
