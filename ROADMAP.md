...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 merged slices: [#196](https://github.com/Hutsoncap/h3code/pull/196) + [#197](https://github.com/Hutsoncap/h3code/pull/197) + [#198](https://github.com/Hutsoncap/h3code/pull/198) + [#199](https://github.com/Hutsoncap/h3code/pull/199) + [#200](https://github.com/Hutsoncap/h3code/pull/200) + [#201](https://github.com/Hutsoncap/h3code/pull/201) — thread-scoped terminal state/store wiring, composer voice lifecycle, runtime/interaction-mode plus persisted thread-settings bindings, the environment-mode binding, the provider-model / traits / fast-mode extraction, and the project-script launch + CRUD bindings all now live on `main`; live lane: `/Users/hutson/Code/h3code-p0.3-chatview-terminal-action-bindings` (`codex/p0.3-chatview-terminal-action-bindings`) / draft PR [#202](https://github.com/Hutsoncap/h3code/pull/202) against `main` — the terminal visibility and close/delete flow now live in `useChatTerminalActionBindings`, local `bun fmt`, `bun lint`, and `bun typecheck` are green, `Test`, `Browser Test`, and `Release Smoke` passed on the latest push, and `Format + Lint`, `Typecheck`, plus `Build Desktop Pipeline` all failed in the shared `Setup workspace` step because `bun install` hit an `electron` postinstall `HTTP 504`; [apps/web/src/components/ChatView.tsx](apps/web/src/components/ChatView.tsx) is down to 5,189 lines on the live branch, so Phase 1 remains blocked until more seams land |
