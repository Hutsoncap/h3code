...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 merged slices: [#196](https://github.com/Hutsoncap/h3code/pull/196) + [#197](https://github.com/Hutsoncap/h3code/pull/197) + [#198](https://github.com/Hutsoncap/h3code/pull/198) + [#199](https://github.com/Hutsoncap/h3code/pull/199) + [#200](https://github.com/Hutsoncap/h3code/pull/200) + [#201](https://github.com/Hutsoncap/h3code/pull/201) + [#202](https://github.com/Hutsoncap/h3code/pull/202) + [#203](https://github.com/Hutsoncap/h3code/pull/203) + [#204](https://github.com/Hutsoncap/h3code/pull/204) — thread-scoped terminal state/store wiring, composer voice lifecycle, runtime/interaction-mode plus persisted thread-settings bindings, the environment-mode binding, the provider-model / traits / fast-mode extraction, the project-script launch + CRUD bindings, the terminal visibility / close-delete flow, the pending approval / user-input response bindings, and the queued-turn / implementation-thread launch bindings all now live on `main`; live lane: `/Users/hutson/Code/h3code-p0.3-chatview-turn-dispatch-bindings` (`codex/p0.3-chatview-turn-dispatch-bindings`) / ready PR [#205](https://github.com/Hutsoncap/h3code/pull/205) against `main` — the turn-start / interrupt / local-dispatch path now lives in `useChatTurnDispatchBindings`, local `bun fmt`, `bun lint`, and `bun typecheck` are green, local `CI=1 bun run test` passes for `apps/server` with the same CI-only serial runner fix that cleared `#204`, [apps/web/src/components/ChatView.tsx](apps/web/src/components/ChatView.tsx) is down to 4,548 lines on the branch versus 5,052 on `main`, the last full CI suite on this head was green, and only PR metadata jobs are rerunning after the base change to `main`; Phase 1 remains blocked until more seams land |
