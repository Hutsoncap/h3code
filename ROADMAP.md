...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 merged slices: [#196](https://github.com/Hutsoncap/h3code/pull/196) + [#197](https://github.com/Hutsoncap/h3code/pull/197) + [#198](https://github.com/Hutsoncap/h3code/pull/198) — thread-scoped terminal state/store wiring, composer voice lifecycle, and runtime/interaction-mode plus persisted thread-settings bindings now live on `main`; live lane: `/Users/hutson/Code/h3code-p0.3-chatview-env-mode-bindings` (`codex/p0.3-chatview-env-mode-bindings`) / draft PR [#199](https://github.com/Hutsoncap/h3code/pull/199) against `main` — the `onEnvModeChange` extraction is fully green and merge-ready after its restack onto current `main`; next stacked slice is active in `/Users/hutson/Code/h3code-p0.3-chatview-composer-model-bindings` (`codex/p0.3-chatview-composer-model-bindings`) / draft PR [#200](https://github.com/Hutsoncap/h3code/pull/200) against `#199` — the provider-model / traits / fast-mode extraction has local `bun fmt`, `bun lint`, and `bun typecheck` green and CI is now running; [apps/web/src/components/ChatView.tsx](apps/web/src/components/ChatView.tsx) is still ~5.6k lines on the stacked branch, so Phase 1 remains blocked until more seams land |
