...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 merged slices: [#196](https://github.com/Hutsoncap/h3code/pull/196) + [#197](https://github.com/Hutsoncap/h3code/pull/197) + [#198](https://github.com/Hutsoncap/h3code/pull/198) + [#199](https://github.com/Hutsoncap/h3code/pull/199) + [#200](https://github.com/Hutsoncap/h3code/pull/200) + [#201](https://github.com/Hutsoncap/h3code/pull/201) + [#202](https://github.com/Hutsoncap/h3code/pull/202) + [#203](https://github.com/Hutsoncap/h3code/pull/203) + [#204](https://github.com/Hutsoncap/h3code/pull/204) + [#205](https://github.com/Hutsoncap/h3code/pull/205) + [#206](https://github.com/Hutsoncap/h3code/pull/206) + [#207](https://github.com/Hutsoncap/h3code/pull/207) + [#208](https://github.com/Hutsoncap/h3code/pull/208) + [#209](https://github.com/Hutsoncap/h3code/pull/209) + [#210](https://github.com/Hutsoncap/h3code/pull/210) + [#211](https://github.com/Hutsoncap/h3code/pull/211) + [#212](https://github.com/Hutsoncap/h3code/pull/212) + [#213](https://github.com/Hutsoncap/h3code/pull/213) + [#214](https://github.com/Hutsoncap/h3code/pull/214) + [#215](https://github.com/Hutsoncap/h3code/pull/215) + [#216](https://github.com/Hutsoncap/h3code/pull/216) + [#217](https://github.com/Hutsoncap/h3code/pull/217) + [#218](https://github.com/Hutsoncap/h3code/pull/218) + [#219](https://github.com/Hutsoncap/h3code/pull/219) + [#220](https://github.com/Hutsoncap/h3code/pull/220) + [#221](https://github.com/Hutsoncap/h3code/pull/221) + [#222](https://github.com/Hutsoncap/h3code/pull/222) + [#223](https://github.com/Hutsoncap/h3code/pull/223) + [#224](https://github.com/Hutsoncap/h3code/pull/224) — prior slices moved the major ChatView terminal, composer, transcript, dialog, shell, send/dispatch, media, plan/handoff, surface-control, timeline/checkpoint-summary, provider/model/menu-derived-state, composer-discovery/query, runtime/view-lifecycle, and session/view-state derivation seams onto `main`; active code-bearing lane: `/Users/hutson/Code/h3code.worktrees/h3code-p0.3-chatview-controller-shell` (`codex/p0.3-chatview-controller-shell`) against `main` — `apps/web/src/components/ChatView.tsx` is now a thin wrapper that hands off to `apps/web/src/components/chat/ChatViewControllerShell.tsx`, so the remaining controller/orchestration shell lives outside the render entrypoint; local `bun fmt`, `bun lint`, and `bun typecheck` are green in this worktree, and the next step is publishing this larger closeout slice as the live `p0.3` PR |
...
| ID        | Title                                                                                    | Status                                                                                                                                                                                                                                            |
| --------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **v1a**   | Sidebar unify + theme catalog                                                            | 🚧 live lane: [#227](https://github.com/Hutsoncap/h3code/pull/227) in `/Users/hutson/Code/h3code.worktrees/h3code-v1a-theme-catalog` (`feat/v1a-theme-catalog`); merged sidebar-unify slice: [#226](https://github.com/Hutsoncap/h3code/pull/226) |
...
Merged slice: [#226](https://github.com/Hutsoncap/h3code/pull/226) landed the sidebar-unify half of `v1a` on `main`. Live PR [#227](https://github.com/Hutsoncap/h3code/pull/227) now targets the remaining theme-catalog half in `/Users/hutson/Code/h3code.worktrees/h3code-v1a-theme-catalog`.
...
_(Importing the 450-theme Ghostty/iTerm2 corpus is scoped separately as v8c. v1a stays focused on the curated catalog.)_
...
| **hX161**       | WebSocket transport response-error fallback hardening                                                                                       | 🚧 worktree: `/Users/hutson/Code/h3code-hx161-ws-transport-error-quoted-blank` — in-progress patch remains local with PR still pending; last focused `apps/web/src/wsTransport.test.ts` pass was green, but full verification has not been republished |
...
| **hX6**  | IPC schema validation at every boundary — make `packages/contracts` the only source of truth; reject unknown fields in dev                                                                                                    | 🚧 #18/#22/#29/#31/#33/#35/#37/#39/#44/#46 — voice transcription, notifications, editor, shell bridge, notification payload, menu action, confirm-message, desktop update, provider discovery websocket, and keybinding websocket IPC validated; more boundaries remain |
...
| **hX22** | Server readiness idempotence — readiness markers should tolerate duplicate completion and expose monotonic startup snapshots so health gates remain stable under repeated startup signals                                     | ✅ #56 — readiness markers made idempotent                                                                                                                                                                                                                              |
...
| **hX19** | Desktop update IPC contract coverage — exercise `DesktopUpdateStateSchema` and `DesktopUpdateActionResultSchema` so malformed update payloads fail before runtime code depends on them                                        | ✅ #53 — desktop update ipc schema coverage added                                                                                                                                                                                                                       |
| **hX20** | Server contract coverage for config, provider status, and voice transcription shapes — exercise `packages/contracts/src/server.ts` so malformed server snapshots fail before runtime code depends on them                     | ✅ #55 — server contract schema coverage added                                                                                                                                                                                                                          |
| **hX21** | WebSocket push-bus send isolation — keep per-recipient `client.send` failures from aborting later recipients or poisoning later queued jobs                                                                                   | ✅ #54 — push-bus send failures isolated                                                                                                                                                                                                                                |
| **hX23** | Local-storage corruption tolerance — malformed JSON and schema-incompatible persisted values degrade predictably instead of breaking consumers                                                                                | ✅ #57 — persisted local-storage corruption now falls back safely                                                                                                                                                                                                       |
| **hX24** | Provider-health cache/bootstrap refresh stability — share a single in-flight refresh and preserve cached snapshots when foreground refreshes fail or overlap                                                                  | ✅ #58 — provider-health refreshes now preserve cached snapshots during overlap and failed refreshes                                                                                                                                                                    |
| **hX25** | Project contract coverage for search and write payloads — exercise `packages/contracts/src/project.ts` so malformed project search/write inputs and results fail before runtime code depends on them                          | ✅ #59 — project search/write schema coverage added                                                                                                                                                                                                                     |
| **hX26** | Provider discovery contract coverage — exercise `packages/contracts/src/providerDiscovery.ts` so malformed discovery payloads fail before runtime code depends on them                                                        | ✅ #60 — provider discovery schema coverage added                                                                                                                                                                                                                       |
| **hX27** | Thread workspace normalization coverage — exercise shared workspace-root normalization and associated worktree precedence so thread workspace metadata stays deterministic                                                    | ✅ #61 — shared thread workspace normalization coverage added                                                                                                                                                                                                           |
| **hX28** | Server attachment-path boundary hardening — add direct unit coverage for attachment-relative path normalization and resolution so path traversal and malformed attachment URLs stay rejected predictably                      | ✅ #62 — attachment path normalization now rejects malformed and traversal-prone inputs predictably                                                                                                                                                                     |
| **hX30** | Model contract coverage — exercise provider-specific model option schemas and alias/default invariants so malformed model payloads fail before runtime code depends on them                                                   | ✅ #64 — model schema coverage added                                                                                                                                                                                                                                    |
| **hX29** | Shell IPC boundary hardening — extract desktop shell handlers and tighten blank-input contract coverage so external-open and show-in-folder calls fail before runtime side effects                                            | ✅ #63 — desktop shell IPC handlers extracted and blank-input boundaries hardened                                                                                                                                                                                       |
| **hX31** | Base schema contract coverage — exercise shared trimmed-string, integer, and branded-id schemas so malformed base payloads fail before higher-level contracts depend on them                                                  | ✅ #65 — shared base schema coverage added                                                                                                                                                                                                                              |
| **hX32** | Temporary git branch normalization hardening — exercise shared temp-branch helpers so mixed-case legacy `dpcode/*` aliases and prefix-stripping edge cases normalize predictably across git flows                             | ✅ #66 — shared temporary-branch normalization hardened                                                                                                                                                                                                                 |
| **hX34** | Codex CLI version-gating hardening — lock down version parsing, comparison, and upgrade messaging so startup rejects unsupported Codex CLI versions predictably, including prerelease and shortened forms                     | ✅ #67 — codex CLI version parsing and prerelease gating hardened                                                                                                                                                                                                       |
| **hX35** | Worktree handoff intent coverage — exercise shared worktree handoff helpers so create-vs-reuse resolution, trimming, base-branch fallback, and associated-worktree truthiness stay deterministic                              | ✅ #68 — shared worktree handoff resolution normalized and covered                                                                                                                                                                                                      |
| **hX36** | Shared schema JSON utility coverage — exercise `packages/shared/src/schemaJson.ts` decode helpers and formatter paths so valid decodes plus malformed JSON and shape failures stay predictable                                | ✅ #69 — shared schema JSON decode helpers and formatter paths covered                                                                                                                                                                                                  |
| **hX37** | Rotating file sink hardening — add shared coverage for constructor validation, overflow-backup pruning, rotation sequencing, zero-length writes, and write/rotate failure handling                                            | ✅ #70 — shared rotating log sink coverage added                                                                                                                                                                                                                        |
| **hX38** | Shared terminal thread identity coverage — exercise command normalization, incremental input/control handling, sticky provider reconciliation, and fallback visual identity defaults so terminal metadata stays deterministic | ✅ #71 — shared terminal thread identity coverage added                                                                                                                                                                                                                 |
| **hX39** | Browser IPC leaf contract coverage — exercise `BrowserNewTabInputSchema`, `BrowserPanelBoundsSchema`, and `BrowserTabStateSchema` so malformed browser leaf payloads fail before runtime code depends on them                 | ✅ #72 — browser IPC leaf schema coverage added                                                                                                                                                                                                                         |
| **hX40** | Provider session directory hardening — add focused coverage for state-dir resolution, provider name normalization, and deterministic fallback session paths                                                                   | ✅ #73 — provider session directory paths and normalization hardened                                                                                                                                                                                                    |
| **hX41** | Provider status cache hardening — add focused coverage for cache-path resolution, blank or unreadable cache fallbacks, atomic temp-file cleanup, and stable unknown-provider ordering                                         | ✅ #74 — provider status cache helpers hardened                                                                                                                                                                                                                         |
...
Each sub-PR is sized for one agent in one worktree. Genuine parallelism is narrower than it looks — most pairs share sidebar structure, the browser-surface contract, or the pin store. Be conservative. When a single gate item is blocking the roadmap, prefer fewer, larger gate-closing slices over a long chain of micro-PRs.
...
- **Hardening lane items are independent only when no blocking gate item is active.** While `p0.3` is open, hardening runs only when it directly unblocks `p0.3`, fixes a failing check on an already-open roadmap PR, or prevents a proven regression from blocking the roadmap path.
...
3. **While working** — keep changes scoped to the sub-PR. If you discover adjacent work worth doing, note it at the bottom of this file under "Spawned follow-ups" and keep going.
   - If the active item is the current roadmap gate, default to a **larger closing slice**. Do not open another micro-PR unless the larger slice would materially raise merge risk.
4. **Completion gates:**
