...
| **p0.3** | Split ChatView.tsx                                | [.plans/04-split-chatview-component.md](.plans/04-split-chatview-component.md)                   | 🚧 merged slices: [#196](https://github.com/Hutsoncap/h3code/pull/196) + [#197](https://github.com/Hutsoncap/h3code/pull/197) + [#198](https://github.com/Hutsoncap/h3code/pull/198) + [#199](https://github.com/Hutsoncap/h3code/pull/199) + [#200](https://github.com/Hutsoncap/h3code/pull/200) — thread-scoped terminal state/store wiring, composer voice lifecycle, runtime/interaction-mode plus persisted thread-settings bindings, the environment-mode binding, and the provider-model / traits / fast-mode extraction all now live on `main`; live lane: `/Users/hutson/Code/h3code-p0.3-chatview-project-script-bindings` (`codex/p0.3-chatview-project-script-bindings`) / draft PR [#201](https://github.com/Hutsoncap/h3code/pull/201) against `main` — the project-script launch + CRUD bindings are extracted, local `bun fmt`, `bun lint`, and `bun typecheck` are green, and every CI check is now green, so the slice is merge-ready; [apps/web/src/components/ChatView.tsx](apps/web/src/components/ChatView.tsx) is down to 5,277 lines on the live branch, so Phase 1 remains blocked until more seams land |
