...
| **hX22** | Server readiness idempotence — readiness markers should tolerate duplicate completion and expose monotonic startup snapshots so health gates remain stable under repeated startup signals                 | 🚧 worktree: `codex/hx22-readiness-idempotence`                                                                                                                                                                                                                         |
...
| **hX20** | Server contract coverage for config, provider status, and voice transcription shapes — exercise `packages/contracts/src/server.ts` so malformed server snapshots fail before runtime code depends on them | 🚧 worktree: `h3code-hx20-server-contract-coverage`                                                                                                                                                                                                                     |
